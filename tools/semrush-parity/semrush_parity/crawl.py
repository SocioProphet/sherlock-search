...
def _normalize_url(url: str) -> str:
    # Basic normalization only; canonicalization should be handled by audit rules.
...
def fetch_url(url: str, timeout_s: float = 20.0, user_agent: str = "sherlock-audit/0.1") -> FetchResult:
    ...
    try:
        ...
        if ct and "text/html" in ct.lower():
            # best-effort; do not assume utf-8 always
...
def extract_links(html: str, base_url: str) -> tuple[list[dict], list[str]]:
    ...
    Each link record includes href, anchor_text, rel.
    """
...
def crawl_site(base_url: str, max_pages: int = 200, timeout_s: float = 20.0) -> dict:
    """Best-effort crawl starting at base_url.

    Returns:
      {
        'pages': { url: {fetch:..., extract:...} },
        'edges': [ {src, dst, anchor_text, rel} ],
        'stats': {...}
      }
    """
    ...
    while queue and len(seen) < max_pages:
        ...
        if fr.html:
            ...
            rec["extract"] = {
                ...
                if k not in {"text", "links", "internal_urls"}  # keep report compact
            }
