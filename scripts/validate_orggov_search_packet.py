#!/usr/bin/env python3
"""Validate OrgGov search packet contracts."""
...
SCHEMA = ROOT / "schemas/orggov-search-packet.v0.1.schema.json"
EXAMPLE = ROOT / "examples/orggov/search-packet.v0.1.example.json"
...
def validate_invariants(record: dict[str, Any]) -> None:
    ...
    if not record.get("searchDocuments"):
        fail("searchDocuments must be non-empty")
    for doc in record["searchDocuments"]:
        weight = doc.get("evidenceWeight")
        if not isinstance(weight, (int, float)) or isinstance(weight, bool):
            fail(f"{doc.get('docId', '<unknown>')}: evidenceWeight must be numeric")
        if weight < 0.0 or weight > 1.0:
            fail(f"{doc.get('docId', '<unknown>')}: evidenceWeight must be in [0, 1]")
    for key, value in record.get("rankingHints", {}).items():
        if not isinstance(value, (int, float)) or isinstance(value, bool):
            fail(f"rankingHints.{key}: expected number")
...
def main() -> int:
    try:
        ...
        return 2
    print("ok: examples/orggov/search-packet.v0.1.example.json validates")
    print("OK: OrgGov search packet validation passed")
    return 0
