...
def main() -> None:
    ...
    if capinfo.get("name") != "search.backend.lexical":
        fail("capability.name must be search.backend.lexical")
    ok("capd semantics validated")

    rpc = load_yaml(pkg / "rpc" / "lexical.index.v0.yaml")
    ...
    topics = load_yaml(pkg / "topics" / "lexical.index.topics.v0.yaml")
