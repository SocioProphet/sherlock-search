...
def main() -> None:
    ...
    if capinfo.get("name") != "search.backend.graph":
        fail("capability.name must be search.backend.graph")
    ok("capd semantics validated")

    rpc = load_yaml(pkg / "rpc" / "graph.reason.v0.yaml")
    ...
    topics = load_yaml(pkg / "topics" / "graph.reason.topics.v0.yaml")
